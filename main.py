...
async def management_task():
    now = datetime.now()
    print(f"[Task 실행] {now} | weekday={now.weekday()} | hour={now.hour} | iso_week={now.isocalendar()[1]}")
 
    guild = bot.get_guild(MY_GUILD_ID)
    if not guild:
        print("[Task] 길드를 찾을 수 없음")
        return
 
    # A. 리셋 1시간 전: 일요일(6) 23시
    # [수정] 기존 코드는 RESET_DAY(월요일) 기준으로 1시간 전을 계산해서
    # "월요일 23시"를 기다렸는데, 리셋이 "월요일 0시"라 이미 지난 시간이었음.
    # 올바른 1시간 전은 "일요일(6) 23시"
    if now.weekday() == 6 and now.hour == 23:
        is_reset_week = (now.isocalendar()[1] % 2 == 0)
        print(f"[Task] 리셋 1시간 전 체크 | is_reset_week={is_reset_week}")
        ...
        for member in guild.members:
            ...
            if active_role not in member.roles:
                if inactive_role:
                    ...
                    warning_count += 1
                    print(f"[Task] D 역할 부여: {member.name}")
 
        if admin_channel:
            await admin_channel.send(
                f"⏰ **리셋 1시간 전 알림**\n"
                f"미활동자 {warning_count}명에게 '{INACTIVE_ROLE_NAME}' 역할을 부여했습니다. "
                f"잠시 후 데이터가 리셋됩니다."
            )
 
    # B. 정각 리셋: 월요일(0) 0시
    # [수정] 격주 체크 시 "어제(일요일)" 기준으로 iso_week 계산
    # 월요일 0시가 되면 주차가 바뀌어버려서 is_reset_week가 틀어지는 버그 수정
    elif now.weekday() == RESET_DAY and now.hour == RESET_HOUR:
        yesterday_iso_week = (now - timedelta(days=1)).isocalendar()[1]
        is_reset_week = (yesterday_iso_week % 2 == 0)
        print(f"[Task] 정각 리셋 체크 | yesterday_iso_week={yesterday_iso_week} | is_reset_week={is_reset_week}")
        if not is_reset_week:
            return
        ...
        if active_role:
            for member in guild.members:
                if active_role in member.roles:
                    ...
                    print(f"[Task] A 역할 회수: {member.name}")
...
async def on_ready():
    ...
    if guild:
        data = load_data()
        active_role = discord.utils.get(guild.roles, name='A')
